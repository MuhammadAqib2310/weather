...
locationsRouter.get("/search", async (req, res, next) => {
    try {
        ...
        if (!env.MAPBOX_TOKEN)
            return res.json([{ name: q, country: "Pakistan", latitude: 31.5204, longitude: 74.3587 }]);
        ...
        res.json(data.features.map((feature) => ({ name: feature.place_name, longitude: feature.center[0], latitude: feature.center[1] })));
    }
    catch (error) {
        next(error);
    }
